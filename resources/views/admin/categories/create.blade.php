...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h2 class="text-xl font-bold">إضافة تصنيف</h2>
        <a href="{{ route('admin.categories.index') }}" class="btn btn-ghost btn-sm">رجوع للقائمة</a>
    </div>

    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="card bg-base-100 shadow">
        @csrf
        <div class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm text-gray-700 mb-1">Slug</label>
                <input name="slug" value="{{ old('slug') }}" class="input input-bordered w-full" required />
            ...
            <div>
                ...
                <input name="name_ar" value="{{ old('name_ar') }}" class="input input-bordered w-full" required />
            ...
            <div>
                ...
                <input name="name_en" value="{{ old('name_en') }}" class="input input-bordered w-full" />
            ...
            <div>
                ...
                <select name="parent_id" class="select select-bordered w-full">
                    ...
                    <option value="{{ $p->id }}" @selected(old('parent_id')==$p->id)>{{ $p->name_ar }}</option>
                    @endforeach
                </select>
        ...
        <div class="px-6 pb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                ...
                <div class="mt-3">
                    <img id="image_preview" src="" alt="" class="hidden object-cover rounded border" style="width:150px;height:150px" />
                </div>
